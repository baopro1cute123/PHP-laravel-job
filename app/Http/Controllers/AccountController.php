...
class AccountController extends Controller
{
    ...
    public function updateProfile(Request $request){
        ...
        if($validator->passes()){
            ...
            session()->flash('success', 'Profile Updated Successed!');
            return response()->json([
                'status' => true,
                'errors' => []
            ]);
    ...
    public function savedJobs(){
        ...
        $jobApplications = SavedJob::where('user_id', Auth::user()->id)
        ->with('job', 'job.jobType','job.applications')
        ->orderBy('created_at','DESC')
        ->paginate(10);
    ...
    public function removeSavedJobs(Request $request){
        ...
        if($savedJob == null){
            session()->flash('error', 'Job SavedJob not found');
            return response()->json([
                'status'=> false
            ]);
        }
    ...
    DB::table('password_reset_tokens')->insert([
        ...
        return redirect()->route('account.forgotPassword')->with('success','Please check your index');
    }
